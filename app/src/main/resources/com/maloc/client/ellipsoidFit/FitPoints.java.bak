package com.maloc.client.ellipsoidFit;
...
public class FitPoints
{
	...
	public Matrix getInvertedW()
	{
		double radii[]=this.radii.toArray();
		double Bfield=Math.pow(radii[0]*radii[1]*radii[2], 1.0/3);
		double evecsD[][]=new double[3][3];
		evecsD[0]=this.evecs.toArray();
		evecsD[1]=this.evecs1.toArray();
		evecsD[2]=this.evecs2.toArray();
		Matrix evecsMatrix=new Matrix(evecsD).transpose();
		double evals[][]=new double[3][3];
		for(int i=0;i<3;i++)
			evals[i][i]=Math.sqrt(this.evals[i]);
		Matrix evalsMatrix=new Matrix(evals).transpose();
		
		Matrix temp=new Matrix(evecsMatrix.getArrayCopy());
		Matrix inverseEvecs=temp.inverse();
		
		Matrix WInverted=evecsMatrix.times(evalsMatrix).times(inverseEvecs).times(Bfield);
